...
func KnapSack(W int, wt []int, val []int, result *int, wg *sync.WaitGroup ) int { 
	...
	if wt[last] > W { 
		return KnapSack(W, wt[:last], val[:last],result,wg)	 
	...
	// (2) item not included 
		...
		wg1.Add(2)
		go func(W int, wt []int, val []int, result *int, wg *sync.WaitGroup,value int ){
			left = value + KnapSack(W , wt, val,result,wg)
			wg.Done()

		}(W - wt[last], wt[:last], val[:last],result,&wg1,val[last])
		go func(W int, wt []int, val []int, result *int, wg *sync.WaitGroup ){
			right = KnapSack(W , wt, val,result,wg)
			wg.Done()

		}(W, wt[:last], val[:last],result,&wg1)
...
func main()  { 
    ...
    go KnapSack(W - weights [last], weights [:last], values [:last],&ok1,&wg)
   // best solution without the last item
    go KnapSack(W, weights [:last], values [:last],&ok2,&wg)
